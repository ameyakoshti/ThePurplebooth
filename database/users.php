...
function get_user_info($user_name) {
	$data = mysql_fetch_assoc(mysql_query("select * from `thepurplebooth`.`users` where `user_name` = \"$user_name\""));
	return $data;
}
...
function get_user_info_by_id($user_id) {
	$data = mysql_fetch_assoc(mysql_query("select * from `thepurplebooth`.`users` where `user_id` = \"$user_id\""));
	return $data;
}
...
function upload_profile_picture($user_id, $file_name, $tmp_name, $file_size, $file_type) {
	try {
		$file_location = $_SERVER["DOCUMENT_ROOT"] . "/thepurplebooth/avatars/" . $user_id . ".jpg";
		$file_location_db = "/thepurplebooth/avatars/" . $user_id . ".jpg";
		move_uploaded_file($tmp_name, $file_location);

		$query = "UPDATE `thepurplebooth`.`users` SET `profile_picture`='$file_location_db' WHERE `user_id`='$user_id'";
